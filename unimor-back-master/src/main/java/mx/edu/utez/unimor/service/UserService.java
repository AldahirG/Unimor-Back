...
public class UserService implements UserDetailsService {
    ...
    public ResponseEntity<?> findAllNoStatus(Pageable pageable) {
        try{
            return new ResponseEntity<>(response(1, userRepository.findAllPaginatedNoStatus(pageable)), HttpStatus.OK);
        }catch (Exception e){
    ...
    @Transactional
    public Object changeUserStatus(Long id){
        try{
            ...
            return new ResponseEntity<>(response(2, "Error al cambiar el estado del usuario"), HttpStatus.OK);
    ...
    public Object uploadProfilePicture(MultipartFile file, Long id){
        if (!file.isEmpty()){
            ...
            Path filePath = Paths.get(ResponseMessage.FILES_LOCATION).resolve(fileName).toAbsolutePath();
            try {
